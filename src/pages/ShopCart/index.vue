      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteCartBtId(cart)">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      // type ：为了区分这三个元素 // disNum形参： 变化量 input最后的个数（不是变化量）
      // cart ：哪一个产品（身上有id）
      switch (type) {
        //加号
        case 'add':
          ...
          break
        case 'minus':
          // 判断产品的个数大于1才可以传递服务器-1 // 如果产品个数小于1，就传递0 让产品数量不变
